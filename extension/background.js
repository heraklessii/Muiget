/**
 * Muiget tarayıcı uzantısı — arka plan servisi.
 *
 ...
 *
 * MV3 service worker'ı (Firefox'ta olay sayfası) her an uykuya alınabilir, bu
 * yüzden hiçbir durum bellekte tutulmuyor; ayarlar `storage.local`'da.
 *
 * Dosya hem Chrome/Edge'de servis çalışanı hem Firefox'ta olay sayfası olarak
 * yükleniyor; bu yüzden `import`/`export` yok — modül olmayan bir bağlamda
 * `export` doğrudan sözdizimi hatası olurdu (karar #31).
 */

/**
 * Tarayıcı API'si.
 *
 * Firefox'ta `browser.*` çağrıları Promise döndürüyor, `chrome.*` yalnızca geri
 * çağrı alıyor. Bu dosya iki biçimi de kullanıyor, o yüzden Firefox'ta
 * `browser` seçiliyor: Firefox'un `browser.*` uygulaması geri çağrı verildiğinde
 * onu da çağırıyor, verilmediğinde Promise döndürüyor. Chrome/Edge'de `browser`
 * yok, `chrome` kalıyor.
 */
...
async function ayarlariAl() {
  const kayitli = await api.storage.local.get(VARSAYILAN_AYARLAR);
...
function kopruyeGonder(mesaj) {
  return new Promise((resolve, reject) => {
    api.runtime.sendNativeMessage(HOST, mesaj, (yanit) => {
 ...
 */
async function indirmeyiGonder({ url, fileName, referrer }) {
  if (!/^https?:\/\//i.test(url)) {
...
async function cerezBasligi(url) {
  if (!api.cookies) return null;
  try {
    const cerezler = await api.cookies.getAll({ url });
...
async function rozetGoster(metin, renk) {
  await api.action.setBadgeText({ text: metin });
  await api.action.setBadgeBackgroundColor({ color: renk });
...
async function videoKaydet(tabId, giris) {
  ...
  const kayit = await api.storage.session.get(anahtar);
  ...
  await api.storage.session.set({ [anahtar]: liste.slice(0, VIDEO_SINIRI) });

  // Sekmeye özel rozet: geçici "✓ / !" rozetleri genel olduğu için
  // birbirlerini ezmiyorlar.
  await api.action.setBadgeText({ tabId, text: String(Math.min(liste.length, 9)) });
  await api.action.setBadgeBackgroundColor({ tabId, color: '#7c5cff' });
}

/** Dinleyiciyi kurar. İzin verilmemişse sessizce vazgeçiyor. */
function videoDinleyiciyiKur() {
  if (!api.webRequest?.onBeforeRequest) return false;
  if (api.webRequest.onBeforeRequest.hasListener(videoYakala)) return true;

  try {
    api.webRequest.onBeforeRequest.addListener(videoYakala, { urls: ['<all_urls>'] });
    return true;
  } catch (e) {
...
async function videoYakalamaAcikMi() {
...
async function sekmeninVideolari(tabId) {
  const kayit = await api.storage.session.get(videoAnahtari(tabId));
...
api.tabs.onUpdated.addListener((tabId, changeInfo) => {
  ...
  void api.storage.session.remove(videoAnahtari(tabId));
...
api.runtime.onMessage.addListener((mesaj, _sender, sendResponse) => {
    ...
    try {
      switch (mesaj?.type) {
        ...
        case 'getSettings':
          ...
          break;
        case 'setSettings':
          await api.storage.local.set(mesaj.payload);
          sendResponse(await ayarlariAl());
          break;
        case 'getVideos':
          sendResponse({
